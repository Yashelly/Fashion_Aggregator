...
Server- and browser-side utility modules: the Supabase-first catalog with bundled CSV fallback, internal-to-public store identity mapping, i18n copy/formatting, optional Supabase clients, and optional PostHog analytics capture. Nothing here has a hard external dependency — every optional integration has a bounded fallback/no-op path.
...
| `catalog.ts` | Server-only Supabase-first storefront loader. Reads only the `catalog_products` public/RLS view with a 1.5s timeout, validates safe IDs and prices, caches successful reads for 60s, and falls back to `getMockProducts()` without caching remote failure as catalog data. |
...
| `supabase-server.ts` | `"server-only"`. Provides the lazy admin client (`getSupabaseServerClient()`, service role) and a separate least-privilege public/RLS client (`getSupabasePublicServerClient()`, publishable/anon key) for search RPC reads. Both return `null` when unconfigured. Never import this module from a client component. |
