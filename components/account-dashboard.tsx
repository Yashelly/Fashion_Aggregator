...
export function AccountDashboard({ locale }: { locale: Locale }) {
  ...
  const styles = isLt
    ? ["Minimal", "Gatvės", "Klasikinis", "Sportinis", "Romantiškas"]
    : ["Minimal", "Streetwear", "Classic", "Sport", "Romantic"];
        ...
        <div className="account-card-grid">
          <section className="account-card">
            ...
            <div className="account-fields size-fields">
              ...
              <label><span>{isLt ? "Avalynė" : "Shoes"}</span><select value={profile.shoeSize} onChange={(event) => update("shoeSize", event.target.value)}>{["36", "37", "38", "39", "40", "41", "42", "43", "44"].map((size) => <option key={size}>{size}</option>)}</select></label>
            </div>
          </section>
          <section className="account-card">
...
@media(max-width:560px){.account-fields.two-column,.account-fields.size-fields{grid-template-columns:1fr}.account-actions{align-items:stretch;flex-direction:column-reverse}.account-actions .button{width:100%}}
`;
