...
- **Search operations** (`lib/search-runtime.ts`, `lib/search-runtime-cache.ts`, `scripts/check-production-search.mjs`): the page-level wrapper logs only mode/cache/latency/result-count metadata, passes the same bounded diagnostics to PostHog, coalesces identical in-flight searches, and caches only results proven to differ from deterministic fallback. A scheduled low-cost production probe runs every six hours; manual runs execute all three hybrid-only checks. Never add raw query text to runtime logs.
- **Feed imports** (`scripts/import-feed.mjs`, `scripts/feed-import-core.mjs`, `scripts/feed-import-postgres.mjs`): provider field aliases live in versioned JSON profiles under `data/feed-configs/`; dry-run is the default. Apply mode requires explicit `--apply`, a store slug, and `SUPABASE_DB_URL`, then verifies store/program permissions before writing `feed_import_runs`, `raw_feed_items`, and `products` transactionally. CI applies migrations 001–003 to a disposable PostgreSQL 17 `weft_test` database and verifies insert, identical re-import, update, full-snapshot removal/restoration, and skipped-row restoration. Never log resolved feed URLs or authorization values. The committed fixture is synthetic and does not replace `data/mock_products.csv` as the public catalog.
