...
from weft_feed_importer;
revoke all on function private.sync_catalog_product_trigger()
from weft_feed_importer;
revoke all on function private.sync_catalog_store_trigger()
from weft_feed_importer;
...
to weft_feed_importer;
grant execute on function private.sync_catalog_product_trigger()
to weft_feed_importer;
...
create policy "Feed importer manages eligible raw items"
  ...
  using (
    exists (
      ...
      join public.stores on stores.id = feed_import_runs.store_id
      where feed_import_runs.id = raw_feed_items.import_run_id
        and feed_import_runs.store_id = raw_feed_items.store_id
        and (
          (feed_import_runs.source_type = 'manual_mock' and stores.public_listing_status = 'demo')
          or (feed_import_runs.source_type = 'affiliate_feed' and stores.affiliate_status = 'approved_feed' and stores.feed_status in ('available_verified', 'importing'))
          or (feed_import_runs.source_type = 'direct_partner_feed' and stores.affiliate_status = 'direct_permission' and stores.feed_status <> 'paused')
        )
    )
  )
  with check (
...
drop policy if exists "Feed importer manages eligible variants"
