...
<!-- ============================================================ -->
<!-- Everything below this line is project documentation for the  -->
<!-- VIBEWEAR codebase, generated by the deepinit skill. It is     -->
<!-- independent of the OMX/Codex runtime content above.           -->
<!-- ============================================================ -->
<!-- Generated: 2026-07-30 | Updated: 2026-07-30 -->

# VIBEWEAR (project documentation)

## Purpose

A pre-affiliate Next.js (App Router) fashion discovery/search MVP for Lithuanian shoppers ("VIBEWEAR"). Shoppers can search/filter a **synthetic demo catalog** (no live retailer products), browse neutral demo stores, and preview a product detail page — but no page redirects to a real merchant or completes a purchase. See `CLAUDE.md` (repo root) for the canonical architecture/commands writeup and `DESIGN.md` for the full brand/UX contract; this section is the deepinit hierarchy's entry point and defers to those two files for depth rather than duplicating them.

## The demo-data boundary (read this before touching product/store data or `/out`)
...
| File | Description |
|------|-------------|
| `CLAUDE.md` | Primary architecture/commands doc for Claude Code — build/lint/test commands, i18n system, analytics flow, env setup. Read first. |
| `DESIGN.md` | Brand/UX/accessibility/content-voice contract ("design contract"). Authoritative for anything visual or copy-tone related. |
| `README.md` | Project overview, first-wave affiliate targets, local dev, locale regression test instructions, DB/env notes. |
| `package.json` | Scripts: `dev`, `build`, `start`, `lint`/`typecheck` (both are `tsc --noEmit`), `test:locale` (Python Playwright suite). |
...
| `sql/` | Numbered Postgres/Supabase migrations for the pre-affiliate schema and analytics tables (see `sql/AGENTS.md`). |
| `worker/` | Empty directory — no files, no subdirectories currently. |

## For AI Agents

### Working In This Directory
- This project-documentation section (below "## Setup") is maintained by the deepinit skill / manual edits when the codebase changes; the content above it is owned by the OMX/Codex CLI setup tooling (`omx setup`) — do not remove or restructure the OMX sections when updating project docs, and vice versa.
- Follow the demo-data boundary above for any change touching `data/*.csv`, `lib/mock-products.ts`, `lib/demo-stores.ts`, or `app/out/[productId]/`.
- `CLAUDE.md` and this file overlap by design — `CLAUDE.md` is the primary onboarding doc; this hierarchy exists for directory-local detail. If the two ever disagree, treat `CLAUDE.md` plus the actual code as authoritative and fix this file.

### Testing Requirements
- `npm run build` (typecheck + production build) and `BASE_URL=http://127.0.0.1:3000 npm run test:locale` (Playwright, requires `npm run dev` running separately) are the two available verification gates. There is no unit-test framework configured.
